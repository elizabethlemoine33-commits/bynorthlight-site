---
layout: post
title: "Shadow AI Is the New Shadow IT — What Leaders Need to Know"
description: "98% of organisations have employees using unapproved AI tools. The risk isn't the technology — it's not knowing what's happening. Here's how to audit it."
og_title: "Shadow AI Is the New Shadow IT — What Leaders Need to Know"
og_description: "98% of organisations have employees using unapproved AI tools. The risk isn't the technology — it's not knowing what's happening. Here's how to audit it."
date: 2026-01-01
date_modified: "2026-07-09"
published_time: "2026-01-01T00:00:00.000Z"
permalink: /blog/shadow-ai-is-the-new-shadow-it.html
source_label: "Blog · Essay"
source_class: source-essay
read_time: 6
download_label: "Download"
download_title: "Shadow AI Governance Checklist"
download_desc: "The four-question audit and follow-up framework for surfacing unsanctioned AI use in your organisation. No sign-up required."
download_url: /blog/shadow-ai-governance-checklist-printable.html
tags:
  - ai governance
  - shadow ai
  - risk management
  - organizational strategy
related_reading:
  - title: "How to Prepare Your Business for AI Adoption: A Canadian Checklist"
    url: /blog/ai-adoption-checklist-canadian-businesses-2026.html
  - title: "Implementing AI Without Breaking Canadian Privacy Law: A PIPEDA Guide"
    url: /blog/pipeda-privacy-first-ai-canada.html
  - title: "Your Agents Need a Chain of Command"
    url: /blog/your-agents-need-a-chain-of-command.html
  - title: "Your AI Isn't Underperforming. It's Working Blind."
    url: /blog/ai-isnt-underperforming-working-blind.html
faq:
  - q: "What does shadow AI actually look like in a small business?"
    a: "Your marketing manager pastes customer lists into ChatGPT to draft segmentation copy. Your finance analyst runs forecasting data through a personal Claude account. Three developers use GitHub Copilot on company code through their personal Gmail. None asked permission — because nobody told them what was acceptable, what data could leave the perimeter, or who to ask. Seventy-eight per cent of AI users bring their own tools to work. They're solving problems now, not waiting for IT."
  - q: "Why do standard responses to shadow AI fail?"
    a: "Banning it drives behaviour underground — people use phones and home wifi; you've traded visible risk for invisible risk. Writing a 40-page policy changes nothing — the people asking permission keep asking; the others still don't. Waiting to see if it resolves itself is already too late. All three responses fail because they assume you know what's happening. You don't. That's the actual problem to solve."
  - q: "What is the real cost of shadow AI in your organisation?"
    a: "IBM's 2025 Cost of a Data Breach Report found shadow AI-involved breaches cost an average of $4.63 million — $670,000 more than breaches without unsanctioned tools — and took 247 days to detect on average. It's a clarity failure, not a technology failure. The breach happened because nobody had an inventory of which tools people were using, what data went into them, or who owned the decision about whether that was acceptable."
  - q: "What does a shadow AI audit actually involve?"
    a: "Four questions: What AI tools are people actually using right now? What data are they putting into those tools? Who is the actual human being who can say yes or no on whether that's acceptable? What is the one next decision to make? You can run this in an afternoon. No consultants, no framework, no deck."
  - q: "Where should leaders start when addressing shadow AI?"
    a: "Start with the audit, not a policy. Find out what's actually happening before you try to govern it. Once you have the inventory — tools in use, data types going in, named decision owner — the path forward becomes visible. Shadow AI isn't going away. The next wave — agentic AI that takes actions, not just generates text — makes this ten times harder if you haven't sorted the basics."
---

<p style="font-size:15px;color:#B0B8D8;background:rgba(79,195,200,0.07);border-left:3px solid #4FC3C8;padding:12px 16px;border-radius:4px;margin:0 0 28px 0;line-height:1.6;">Northlight Advisory Services, a Halifax, Nova Scotia–based strategy and AI advisory practice, breaks down why shadow AI has replaced shadow IT as the governance risk most leaders haven't named yet — and how to surface it without a policy or a ban.</p>

Ninety-eight per cent of organisations have employees using AI tools nobody approved. Your organisation is almost certainly one of them. The question isn't whether to stop it — you can't — it's whether you know what's happening, who owns it, and what data is going where. Most leaders I talk to don't. That's not a discipline problem. That's a clarity problem.

## What Does Shadow AI Actually Look Like in a Small Business?

<div class="answer-block"><p>Your marketing manager pastes customer lists into ChatGPT to draft segmentation copy. Your finance analyst runs forecasting data through a personal Claude account. Three developers use GitHub Copilot on company code through their personal Gmail. None asked permission. None think they're doing anything wrong — because nobody told them what was acceptable, what data could leave the perimeter, or who to ask. Seventy-eight per cent of AI users bring their own tools to work. They're solving problems now, not waiting for IT.</p></div>

Let me be specific about what I mean when I say [shadow AI](https://www.ibm.com/think/topics/shadow-ai).

Your marketing manager pastes customer email lists into ChatGPT to draft segmentation copy. Your finance analyst runs Q3 forecasting data through a personal Claude account to catch anomalies faster. Your dev team uses GitHub Copilot on company code — except three of them are on the free tier with their personal Gmail addresses, not your enterprise subscription. None of them asked permission. None of them think they're doing anything wrong. And they're not wrong to think that — because nobody ever told them which tools were okay, what data they could put in, or who to ask.

This isn't hypothetical. [Seventy-eight per cent of AI users bring their own tools to work](https://cybernews.com/ai-news/bring-your-own-ai-rise-shadow-ai-workplace/). They're not waiting for your IT team to provision something. They're solving problems now, with the free-tier LLM that's one browser tab away.

And here's the part that makes this hard: they're probably getting better results than they would if they waited for you to catch up.

## Why Do Standard Responses to Shadow AI Fail?

<div class="answer-block"><p>Banning it drives behaviour underground — people use phones and home wifi; you've traded visible risk for invisible risk and told your best people that solving problems faster makes them rule-breakers. Writing a 40-page policy changes nothing — the people asking permission keep asking; the others still don't. Waiting to see if it resolves itself is already too late. All three responses fail for the same reason: they assume you know what's happening. You don't. That's the actual problem to solve.</p></div>

The first instinct is to ban it. Block ChatGPT at the firewall. Send an email that says "unapproved AI tools are prohibited effective immediately." I've watched organisations do this. It doesn't work. It just drives the behaviour underground — now people use their phones, their home wifi, their personal laptops. You've traded visible risk for invisible risk, and you've told your best people that solving problems faster makes them rule-breakers.

The second instinct is to write a policy. Forty pages, definitions on page three, a framework for acceptable use, a review process that requires three signatures and two weeks. Nobody reads it. The people who would've asked permission keep asking. The people who weren't asking still aren't. You've spent a quarter on a document that changes nothing.

The third instinct is to wait and see — maybe this is a fad, maybe it'll regulate itself, maybe we'll have clarity in six months. That one's already too late. The agentic AI wave is coming; organisations that haven't sorted out what data can leave their perimeter and who owns that decision are going to be making it under pressure, in the middle of an incident, with regulators watching.

None of these work because they all start from the wrong place. They assume you know what's happening and need to control it. You don't know what's happening. That's the actual problem.

## What Is the Real Cost of Shadow AI in Your Organisation?

<div class="answer-block"><p>IBM's 2025 Cost of a Data Breach Report found shadow AI-involved breaches cost an average of $4.63 million — $670,000 more than breaches without unsanctioned tools — taking 247 days to detect on average. That's not a technology failure. It's a clarity failure. The breach happened because nobody had an inventory of which tools people were using, what data went into them, or who owned the decision about whether that was acceptable.</p></div>

I'm not going to pretend there's no risk here. There is.

[The IBM 2025 Cost of a Data Breach Report](https://www.vectra.ai/topics/shadow-ai) found that breaches involving shadow AI cost an average of $4.63 million — $670,000 more than breaches that didn't involve unsanctioned tools. It took organisations an average of 247 days to detect those breaches. That's not a small number, and it's not a hypothetical scenario. It's what happens when someone puts regulated data into a tool that logs it, stores it, or trains on it — and nobody knew the tool existed until the breach notification landed.

But here's what I want you to notice about that number: it's not a technology failure. It's a clarity failure.

> The breach didn't happen because AI is inherently insecure. It happened because nobody knew which tools people were using, what data was going into them, or who owned the decision about whether that was acceptable.

The organisation didn't have an inventory. They didn't have a named owner. They didn't have an answer to the question "what data can leave our perimeter, and under what conditions?"

That's fixable. Not with a ban. Not with a policy. With an audit.

## What Does a Shadow AI Audit Actually Involve?

<div class="answer-block"><p>Four questions: What AI tools are people in this organisation actually using right now (not what's approved — what's actually running)? What data are they putting into those tools, specifically? Who is the actual human being who can say yes or no on whether that data exposure is acceptable? What is the one next decision we need to make? You can run this in an afternoon. You don't need to hire anyone. You need willingness to ask the questions and authority to act on what you find.</p></div>

An audit isn't a framework. It's not a maturity model. It's not a consultant-led engagement that takes three months and produces a deck.

It's four questions:

1. **What AI tools are people in this organisation actually using?** Not "what tools are approved" — what tools are people using right now, today, whether you know about it or not.
2. **What data are they putting into those tools?** Customer lists, financial forecasts, strategy decks, code, employee records, contract terms — name it specifically.
3. **Who owns the decision about whether that's acceptable?** Not "the policy says..." — who is the actual human being who can say yes or no, and what information do they need to make that call?
4. **What's the next decision we need to make?** Do we need to provision an enterprise version of a tool people are already using? Do we need to pull certain data types out of scope? Do we need to name an AI governance owner who doesn't exist yet? One decision. Not a roadmap. Not a five-year plan. The next decision.

> You can run this audit in an afternoon. You don't need to hire anyone. You need one person who's willing to ask the questions, a spreadsheet, and the organisational authority to say "we're going to name what's happening, and then we're going to decide what to do about it."

That person is probably you.

<div class="callout">I ran this audit on Northlight Advisory Services' own stack — a one-person operation — and found 10 AI tools across 5 functional areas in under 90 minutes, including one I kept meaning to use and one I'd tried and decided wasn't for me. If that's what a solo practice looks like, imagine the sprawl in a team of 20.</div>

## Where Should Leaders Start When Addressing Shadow AI?

<div class="answer-block"><p>Start with the audit, not a policy. Find out what's actually happening before you try to govern it. Once you have the inventory — tools in use, data types going in, named decision owner — the path forward becomes visible. Northlight Advisory Services' AI Governance Readiness Checklist walks through the questions to ask, the format to capture answers, and the follow-up decision tree. Shadow AI isn't going away. The next wave — agentic AI that takes actions, not just generates text — makes this ten times harder if you haven't sorted the basics.</p></div>

I built a governance audit checklist — the actual questions to ask, the format to capture answers in, and the follow-up decision tree that tells you what to do with what you find. It's designed for someone who's running this audit for the first time, in an organisation that doesn't have an AI policy yet and isn't sure it needs one.

[Download the Shadow AI Governance Checklist →](/blog/shadow-ai-governance-checklist-printable.html)

Shadow AI isn't going away. The tools are too useful, the behaviour is too embedded, and the next wave — agentic AI that takes actions on your behalf, not just generates text — is going to make this ten times harder if you don't have the basics sorted now.

You don't need a 40-page policy. You don't need a ban. You need to know what's happening. Start there.

If you're trying to figure out whether Northlight Advisory Services is the right fit for where your organisation is right now, [start here](/who-we-help.html).
